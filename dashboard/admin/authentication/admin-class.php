...
require_once ___DIR__. '/../../../src/vendor/autoload.php';
...
class ADMIN
{
    ...
    {
        $this->settings = new SystemCongig();
        $this->smtp_email = $this->settings->getSmtpEmail();
        $this->smtp_password = $this->settings->detSmtpPassword();
...
public function verifyOTP($username, $email, $password, $tokencode, $otp, $csrf_token){
    if($otp == $_SESSION['OTP']){
        ...
        <head>
            <meta charset='UTF-8'>
            <title>OTP Verifecation success</title>
            <style>
                body {
                font-family: Arial, sans-serif;
                background-color: #f5f5f5;
        ...
        <body>
            <div class='container'>
                ...
                <p>Welcome to Itelec System</p>
                <p>If you did not sign up for an account, you can safely ignore this emai.</p>
                <p>Thank you!</p>
            </div>
        </body>
        </html>";
            ...
            if($uuserRow->rowCount() == 1){
                if($userRow['status' == "active"]){
                    if($userRow['password'] == md5($password)){
                        ...
                        echo "<script>alert('Password is incorrect'); window.location.href = 'http://localhost/ITELEC2-V2/dashboard/admin/authentication/';</script>";
                        exit;
                    }
                }else{
                    echo "<script>alert('Entered Email is not verify'); window.location.href = 'http://localhost/ITELEC2-V2/dashboard/admin/authentication/';</script>";
                    exit;
                }
            }else{
                echo "<script>alert('No Account found'); window.location.href = 'http://localhost/ITELEC2-V2/dashboard/admin/authentication/';</script>";
                    exit;
            }
